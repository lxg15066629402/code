    ...
    for nii_file in os.listdir(nii_floder):
        ...
        if nii_file.split('.')[-1] == 'gz':
            ...
            for nii in range(len(nii_all)):
                ...
                temp_img = img_data.get_fdata()
                print(nii_file_name)
                print(temp_img.shape)
                # # exit()
                # print("=====")
                print(temp_img.shape[2])
                # exit()
                if temp_img.shape[2] == img.shape[2]:
                    # img = cv2.bitwise_or(img, temp_img)
                    img = np.add(img, temp_img)
                else:
                    continue
                # print(img)
                print(img.shape)
                ...
                #     ret, temp_binary = cv2.threshold(mask, 0, 255, cv2.THRESH_BINARY)
                #     # 转化类型
                #     img = np.zeros((2016, 2816))
                #     # print(type(binary))  # <class 'numpy.ndarray'>
                #     binary = cv2.bitwise_or(img, temp_binary)
