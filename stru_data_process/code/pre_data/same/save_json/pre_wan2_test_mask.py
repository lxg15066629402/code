...
def compare_double(folders):
    ...
    for file in os.listdir(folders):
        ...
        # 获取nii信息, 检索信息
            ...
            if 'wy' in split_name or 'WY' in split_name or "w y" in split_name:
                nii_file = os.path.join(folders, file)
                nii_type = int(file.split('_')[0])
                nii_mask_index = int(file.split('_')[1])
                if nii_type == 2:
                    if 'doctor_wy' not in nii_dic.keys():
                        nii_dic['doctor_wy'] = {}

                    img = get_one_nii(nii_file)
                    nii_dic['doctor_wy'][f'{nii_type}_{nii_mask_index - 1}'] = {"img": img}

            elif 'huangyan' in split_name:
                nii_file = os.path.join(folders, file)
                nii_type = int(file.split('_')[0])
                nii_mask_index = int(file.split('_')[1])
                if nii_type == 2:
                    if 'doctor_hy' not in nii_dic.keys():
                        nii_dic['doctor_hy'] = {}

                    img = get_one_nii(nii_file)
                    nii_dic['doctor_hy'][f'{nii_type}_{nii_mask_index - 1}'] = {"img": img}

        else:
            print("数据标注错误,错误文件名为,", file)
            continue
    ...
    if len(doctor_name) == 2:
        ...
        for index_nii in nii_index_list:
            # print(index_nii)
            # exit()
            # print(nii_dic[doctor_name[0]].keys())
            # print(nii_dic[doctor_name[1]].keys())
            # exit()
...
def main():
    ...
    # 01
    # save_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/wan2_json1/data'
    # save_json_path = '/dicom/Jone/code/data_pre/stru_data_process/same/wan2_json1/annotations'
    # mask_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/wan2_json1/ann'
