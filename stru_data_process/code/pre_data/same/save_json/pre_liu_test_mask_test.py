...
# 功能： 实现数据转化，保存成coco 格式的数据，便于mmdetection 深度学习目标检测任务使用, 同时将不规则数据进行保存
"""
...
def get_one_nii_bounding_box(nii_all, save_mask=True):
...
# 将dcm文件和nii文件的信息进行匹配
def connect_pngs_boxs(bounding_box_list, png_dic, contours_list, folder, data_path, mask_path, mask_path_, img_id, ann_id, mask_list,
    ...
    for index_img in range(len(bounding_box_list)):
        ...
        if save_mask == True:
            ...
            cv2.imwrite(os.path.join(mask_path_, file_name), mask_list[index_img])
...
def main():
    ...
    # 01
    save_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/liu_json_masktest/data'
    save_json_path = '/dicom/Jone/code/data_pre/stru_data_process/same/liu_json_masktest/annotations'
    mask_path_ = '/dicom/Jone/code/data_pre/stru_data_process/same/liu_json_masktest/mask'
    mask_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/liu_json_masktest/ann'
    save_json_name = 'liu_json'

    # 创建文件保存路径
    create_dir_not_exists(save_png_path)
    create_dir_not_exists(save_json_path)
    create_dir_not_exists(mask_path_)
    ...
    for doctor in sorted(os.listdir(dcm_folders)):
        ...
        for idx in list(all_nii_dic.keys()):
            ...
            bounding_box_list, contours_list, mask_list = get_one_nii_bounding_box(all_nii_dic[idx])
            ...
            png_list, images, annotations, image_id, ann_id = connect_pngs_boxs(bounding_box_list, png_dic[int(idx)],
                                                                                ...
                                                                                mask_png_path, mask_path_, image_id, ann_id,
                                                                                mask_list, save_png=True, save_mask=True)
