...
def get_nii(nii_floder):
    ...
    for nii_file in os.listdir(nii_floder):
        ...
        type_name = int(nii_file.split('_')[0])

        if nii_file.split('.')[-3] == 'ZB':  #
            img_data = nib.load(nii_file_name)
            img = img_data.get_fdata()

            if img.shape[-1] == 1 and type_name == 2:
                single_nii.append(nii_file)  #

            elif img.shape[-1] != 1:
                dou_nii.append(nii_file)
            else:
                print("脏数据")
                continue

    return single_nii, dou_nii
...
def main():
    ...
    # 01
    # save_png_path = '/dicom/Jone/code/data_pre/stru_data_process/save_data/diff_json1/data'
    # save_json_path = '/dicom/Jone/code/data_pre/stru_data_process/save_data/diff_json1/annotations'
    # mask_png_path = '/dicom/Jone/code/data_pre/stru_data_process/save_data/diff_json1/ann'
    # save_json_name = 'coco_json'

    # save_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/diff_json1/data'
    # save_json_path = '/dicom/Jone/code/data_pre/stru_data_process/same/diff_json1/annotations'
    # mask_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/diff_json1/ann'
    # save_json_name = 'coco_json'

    # 02
    save_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/all_data_all/data'
    save_json_path = '/dicom/Jone/code/data_pre/stru_data_process/same/all_data_all/annotations'
    mask_png_path = '/dicom/Jone/code/data_pre/stru_data_process/same/all_data_all/ann/'
    ...
    for doctor in sorted(os.listdir((dcm_floders))):
        ...
        for floder in os.listdir(patient_path):
            ...
            single_nii, dou_nii = get_nii(patient)
            # print(len(single_nii))
            ...
            if len(bounding_box_list) > 1:
                ...
                png_dic = dcm_png_single(patient, single_nii)
                png_list, images, annotations, image_id, ann_id = connect_pngs_boxs(bounding_box_list,
                                                                                    png_dic,
                                                                                    contours_list,
                                                                                    floder,
